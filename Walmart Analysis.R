...
#DATA READ
store <- read.csv(file='stores.csv')
traindata<- read.csv(file='train.csv')
features  <- read.csv(file='features.csv')
df_train <- data.frame(traindata)
df_trainSub <- subset(df_train, Dept == 90 & Store == 20)
df_trainSub
#CODE
#merge store data with the train data
tdata <- merge(x=traindata, y=store, all.x=TRUE)
#merge features data with the train data
...
#aggregate function to sum all the weekly sales of the different departments
add_weekly_sales <- function(){
  for(n in 1:45){
...
trainHW1 <- hw(train.train1[,3], h = 53)
#Frequency too high 
...
train1Net
train1Netfo <- forecast(trainNet, h = 42)
plot(trainNetfo, ylab ="Weekly Sales")
...
write.csv(output_format, file = "output_team_1.csv")
